...
class CircularQueue:            #원형 큐 클래스 선언
    ...
    def __init__(self, size):   #원형 큐 초기화
        self.front = 0
        self.rear = 0
        self.queuesize = size
        self.data = [None] * size
    ...
    def peek(self):             #Front 다음 위치 원소(제일 먼저 넣은 원소) 출력
        if self.isempty() == True:
            print("공백 원형 큐")
        else:
            print("첫 번째 원소 : ",self.data[(self.front + 1) % self.queuesize])
    ...
    def printqueue(self):       #원형 큐 출력
        if self.isempty() == True:
            ...
            r = self.rear
            print("원형 큐 = ",end="")
            #나머지 연산을 통해 원형으로 돌면서 순서대로 원소 출력
